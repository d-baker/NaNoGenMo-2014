...
def p2():
    ...
    p = "The {noun1} is the {noun2} of {adjective1} {things}.  To reveal {nouns1} and conceal the {noun2} is the {noun1s} aim. Those who find {adjective2} meanings in {adjective1} {things} are {adjective3} without being {adjective4}.".format(
        noun1 = n1,
        noun2 = random.choice(ns).strip(),
        adjective1 = random.choice(adjs).strip(),
        things = pluralize(random.choice(ns).strip()),
        nouns1 = pluralize(random.choice(ns).strip()),
        noun1s = n1 + "'s",
...
def gen():
    ...
    for i in range(0, random.randint(1, 5)):
        text += "\n" + random.choice([opening(), p1(), p2(), p3()]) + "\n"
